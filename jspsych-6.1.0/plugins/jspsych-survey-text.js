    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(question.rows == 1){
        if (trial.required_word) {
          html += '<input type="text" id="input-'+question_index+'" onkeydown="word_length()"  name="#jspsych-survey-text-response-' + question_index + '" data-name="'+question.name+'" size="'+question.columns+'" '+autofocus+' '+req+' placeholder="'+question.placeholder+'"></input>';
        }else{
        ...
        if (trial.required_word) {
        html += '<textarea id="input-'+question_index+'" onkeydown="word_length()" name="#jspsych-survey-text-response-' + question_index + '" data-name="'+question.name+'" cols="' + question.columns + '" rows="' + question.rows + '" '+autofocus+' '+req+' placeholder="'+question.placeholder+'"></textarea>';
        } else {
